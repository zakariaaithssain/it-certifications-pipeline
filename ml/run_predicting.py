from predictor import MissingColumnsPredictor
...
data = pd.read_csv('../data/pre_predictions_data.csv')
...
level_training_data = pd.get_dummies(data=useful_columns, columns=['Provider','Domain'])[(data['Cost (USD)'] > 0)
                                                                                         ...
                                                                                         &(data['Domain'] != 'unknown')
                                                                                         &(data['Level']!= 'unknown')]
level_model = DecisionTreeClassifier(random_state=37)
...
level_features = level_predictor.get_optimal_features()
...
domain_training_data = pd.get_dummies(data=useful_columns, columns=['Provider','Level']).map(lambda domain: 'business apps'
                                                                    if domain=='it/business'
                                                                                         ...
                                                                                         &(data['Domain'] != 'unknown')
                                                                                         &(data['Level']!= 'unknown')]
domain_model = DecisionTreeClassifier (random_state=37)
...
domain_features = domain_predictor.get_optimal_features()
...
cost_features = cost_predictor.get_optimal_features()
...
duration_features = duration_predictor.get_optimal_features()
...
data.to_csv(r'../data/post_predictions_data.csv', index= False)